...
async def tocar(bot, *, query: str):
    ...
    await bot.send(f"🔍 DJ Pinhas está procurando por **`{query}`**...")
    ...
    try:
        ...
        if bot.voice_client.is_playing() or bot.voice_client.is_paused():
            ...
            await bot.send(f"✅ DJ Pinhas adicionou **{nomeMsc}** à fila")
        else:
            FFMPEG_OPTS = {'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5', 'options': '-vn'}
            src = await discord.FFmpegOpusAudio.from_probe(urlMsc, **FFMPEG_OPTS)
            ...
            await bot.send(f"▶️ DJ Pinhas está tocando agora: **{nomeMsc}**")

    except Exception as e:
        await bot.send("Perdoa o Pinhas, não consegui encontrar a música😔😔😭😭😭")
        print(f"Erro no comando !pinhas: {e}")
